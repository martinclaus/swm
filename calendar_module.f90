...
#include "udunits.inc"
            ...
            decstr = TRIM(ADJUSTL(orstr)) // " since " // TRIM(ADJUSTL(yearstr)) // "-" // TRIM(ADJUSTL(monthstr)) // "-" // TRIM(ADJUSTL(daystr)) // " " // TRIM(ADJUSTL(hourstr)) // ":" // TRIM(ADJUSTL(minutestr)) // ":" // TRIM(ADJUSTL(secondstr))
           
            status = UTDEC(decstr, cal%ptr)
            IF (status .NE. 0) THEN
                PRINT *, "UTDEC ERROR: ", status
                STOP 1
            ENDIF
