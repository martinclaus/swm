...
!
  ...
  type(UT_FUNC_PTR) :: f_ut_ignore

  contains

!=============================================================================
  subroutine init_f_udunits_2()
    implicit none

    interface
      integer(c_int) function c_ut_ignore(fmt, va_list) bind(C, name="ut_ignore")
        use iso_c_binding
        type(c_ptr), value :: fmt, va_list
      end function
    end interface

    f_ut_ignore%ptr = c_funloc(c_ut_ignore)
  end subroutine
!=============================================================================
...
!=============================================================================
  type(UT_FUNC_PTR) function f_ut_set_error_message_handler(err_handler)
    use iso_c_binding
    implicit none
    type(UT_FUNC_PTR), intent(in) :: err_handler

    interface
      type(c_funptr) function c_ut_set_error_message_handler(err_handler) bind(C, name="ut_set_error_message_handler")
        use iso_c_binding
        implicit none
        type(c_funptr), value :: err_handler
      end function
    end interface
    f_ut_set_error_message_handler%ptr = c_ut_set_error_message_handler(err_handler%ptr)
  end function
