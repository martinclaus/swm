...
! factors of derivatives in spherical coordinates and coriolis terms
  ...
  FORALL (i=1:Nx, j=1:Ny)
    ...
    lat_mixing_v(i,j,1) = -(ocean_H(ip1(i),j)+ocean_H(i,j))/(dLambda*A*cosTheta_v(j))**2 &
                          - 2/(dTheta*A)**2 + (1-tanTheta_v(j)**2)/(A**2)
    lat_mixing_u(i,j,2) = ocean_H(i,jp1(j))*(1/(dTheta*A)**2 - tanTheta_u(j)/(2*dTheta*A**2))
    lat_mixing_v(i,j,2) = 1/(dTheta*A)**2 - tanTheta_v(j)/(2*dTheta*A**2)
    lat_mixing_u(i,j,3) = ocean_H(i,j)*(1/(dTheta*A)**2+tanTheta_u(j)/(2*dTheta*A**2))
    lat_mixing_v(i,j,3) = 1 /(dTheta*A)**2 + tanTheta_v(j)/(2*dTheta*A**2)
    lat_mixing_u(i,j,4) = 1/(dLambda*A*cosTheta_u(j))**2
    lat_mixing_v(i,j,4) = ocean_H(ip1(i),j)/(dLambda*A*cosTheta_v(j))**2
    lat_mixing_u(i,j,5) = lat_mixing_u(i,j,4)
    lat_mixing_v(i,j,5) = ocean_H(i,j)/(dLambda*A*cosTheta_v(j))**2
