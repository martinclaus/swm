...
def time_integration(u,v,eta):
    ...
    for i in range(param['Nt']):
        ...
        t += dt
        feedback(u, v, eta, t, tic, i)

    print(('Integration done in '+readable_secs(tictoc.time() - tic)
           + ' on '+tictoc.asctime()))
    output_txt(('\nTime integration done in '
                + readable_secs(tictoc.time() - tic)
                + ' on ' + tictoc.asctime()))
...
def feedback_ini(u, v, eta, t):
    if param['output']:
        ...
        output_txt('Integrating %.1f days with dt=%.2f min in %i time steps'
                   % (param['Ndays'], param['dt'] / 60., param['Nt']))
        output_txt('Time integration scheme is ' + param['scheme']
                   + ' with CFL = %.2f' % param['cfl'])
        output_txt('')
        output_txt('Starting shallow water model on '+tictoc.asctime())
        print(('Starting shallow water model run %i on ' % param['run_id'])
              + tictoc.asctime())
    else:
        print('Starting shallow water model on '+tictoc.asctime())


def feedback(u, v, eta, t, tic, i):
